    ...
    static func makeBricArr(from data: Data) -> [BricABrac]? {
        ...
        do {
            ...
            guard let fields = records[0] as? [String: Any] else {
                ...
                guard let itemsSubTitle = fields["title"] as? String else {
                    print("There was an error casting from [String: Any] to String so we couldn't get titles")
                    return nil}
                // titles are complicated
                guard let firstPartOfTitle = fields["object"] as? String else {
                    print("There was an error casting from [String: Any] to String so we couldn't get the first part of titles")
                    return nil}
                guard let secondPartOfTitle = fields["date-text"] as? String else {
                    print("There was an error casting from [String: Any] to String so we couldn't get the second part of titles")
                    return nil}
                guard let thirdPartOfTitle = fields["place"] as? String else {
                    print("There was an error casting from [String: Any] to String so we couldn't get the last part of titles")
                    return nil}
